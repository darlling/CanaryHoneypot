...
def handle(payload, default="page"):
    page = payload["page"]
    page_list = []
    second_page_list = []
    pay = (
        logselect.page_select_attack(page)
        if default == "page"
        else logselect.page_select_white(page)
    )
    for i in pay:
        dict_param = {
            "id": i.id,
            "dst_host": i.dst_host,
            "dst_port": i.dst_port,
            "honeycred": i.honeycred,
            "local_time": i.local_time.strftime("%Y-%m-%d %H:%M:%S"),
            "hostname": i.hostname,
            "password": i.password,
            "path": i.path,
            "skin": i.skin,
            ...
            "data": i.data,
            "function": i.function,
            "vnc_client_response": i.vnc_client_response,
            "vnc_password": i.vnc_password,
            ...
            "headers_from": i.headers_from,
            "headers_to": i.headers_to,
            "headers_via": i.headers_via,
            "community_string": i.community_string,
            "requests": i.requests,
            "urg": i.urg,
            "psh": i.psh,
            "fin": i.fin,
            "appname": i.appname,
            "cltintname": i.cltintname,
            "database": i.database,
            "language": i.language,
    ...
    return {"list": second_page_list}


def listpage(param):
    if "page" in param:
        if "white" in param:
            return handle(param, "white")
        return handle(param)
