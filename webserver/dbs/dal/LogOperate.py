...
class LogOp:
    ...
    def insert(
        ...
        loginsert = OpencanaryLog(
            ...
            data=data,
            function=function,
            vnc_client_response=vnc_client_response,
            vnc_password=vnc_password,
            vnc_server_challenge=vnc_server_challenge,
            inputs=inputs,
            domain=domain,
            headers_call_id=headers_call_id,
            headers_content_length=headers_content_length,
            headers_cseq=headers_cseq,
            headers_from=headers_from,
            headers_to=headers_to,
            headers_via=headers_via,
    ...
    def page_select_attack(self, page_index):
        try:
            ...
            # num = 10*int(page) - 10
            logselect = (
                self.session.query(OpencanaryLog)
                .filter(OpencanaryLog.white == 2)
                .order_by(desc(OpencanaryLog.local_time), OpencanaryLog.id)
                .limit(page_size)
    ...
    def pie_select_num(self, years):
        try:
            pie_num = (
                self.session.query(
                    func.count(OpencanaryLog.logtype), OpencanaryLog.logtype
                )
                .group_by(OpencanaryLog.logtype)
                .filter(
    ...
    def select_attack_total(self):
        try:
            total_attack = (
                self.session.query(func.count(OpencanaryLog.id))
                .filter(OpencanaryLog.white == 2)
                .scalar()
            )
            return total_attack
