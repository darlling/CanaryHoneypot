...
Currently Honeypot supports faking the following services natively:
...
`smb` - a log watcher for Samba logging files which allows Honeypot to alert on files being opened in a Windows File Share.

For this configuration, you will need to setup your own Windows File Share, and point Honeypot at it using the following configuration,
...
`portscan` - a log watcher that works with iptables to monitor when your Honeypot is being scanned.
At this stage the portscan module supports detection of nmap OS, nmap FIN, nmap OS, nmap NULL and normal port scans.

Logger Configuration
--------------------

Honeypot allows us to define a bunch of logging/alerting sinks. Below are a list of options you can simply
    ...
    "logger": {
        ...
        "kwargs": {
            ...
            "handlers": {
                ...
                "slack":{
                    "class":"honeypot.logger.SlackHandler",
                    "webhook_url":"https://hooks.slack.com/services/..."
                },
                "teams": {
                    "class": "honeypot.logger.TeamsHandler",
...
When you generate the default Honeypot config file using,

.. code-block:: sh

    $ honeypotd --copyconfig

you will receive a json formatted config file at `/etc/honeypot/honeypot.conf` such as the following,
