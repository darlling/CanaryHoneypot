...
class RedisProtocol(Protocol):
    ...
    def _buildResponseAndSend(self, input_cmd, input_args):
        ...
        except (UnknownCommandError, ArgumentCountError, AuthenticationError,
    ...
    def _logAlert(self, cmd, args):
        ...
        if len(args) > self.factory.max_arg_length:
            args = args[:self.factory.max_arg_length] + \
                    '(and '+str(self.factory.max_arg_length-len(args))+' more bytes)'
        logdata = {"CMD": cmd,
                   "ARGS": args}
    ...
    def _processRedisCommand(self,):
        ...
        def _parseRESPArray(cmd_string):
            ...
            if cmd_string[0] != '*':
                raise ProtocolError("expected '*', got '{c}'"
                                        .format(c=cmd_string[0]))
        ...
        def _parseRESPString(cmd_string):
            ...
            resp_str = cmd_string[curr_ptr:(curr_ptr+str_length+2)] #string + CRLF
